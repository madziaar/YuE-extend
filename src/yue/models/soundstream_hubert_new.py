...
class SoundStream(nn.Module):
    ...
    def __init__(
        ...
        self.decoder_semantic = Decoder(
            code_dim=768, output_channels=768, decode_channels=768
        )
        # out_D=D+768
    ...
    def forward(self, x: torch.Tensor, bw: int):
        ...
        semantic_recon_loss = F.mse_loss(
            e_semantic_input.transpose(1, 2).detach(), o_semantic
        )
    ...
    def encode(self, x: torch.Tensor, target_bw: Optional[int] = None) -> torch.Tensor:
        ...
        if e_acoustic.shape[2] != e_semantic.shape[2]:
            ...
            e_acoustic = self.encoder(
                torch.transpose(F.pad(x[:, 0, :], (160, 160)).unsqueeze(0), 0, 1)
            )
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        o, commit_loss, distill_loss, _ = soundstream(
            x, soundstream.target_bandwidths[-1]
        )
