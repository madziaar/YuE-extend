...
parser.add_argument(
    "--genre_txt",
    type=str,
    default="prompt_examples/genrerock.txt",
...
parser.add_argument(
    "--lyrics_txt",
    type=str,
    default="prompt_examples/lastxmas.txt",
...
parser.add_argument(
    "--server_port", type=int, default=7860, help="The port number for the WebUI."
)
...
def stage1_inference(
    ...
    song_id = get_song_id(
        seed, genres, top_p, temperature, repetition_penalty, max_new_tokens
    )
...
def stage2_generate(
    model, prompt, batch_size=16, segment_duration=6, state=None, callback=None
):
...
def generate_song(
    ...
    if use_icl:
        if prompt_start_time > prompt_end_time:
            raise gr.Error("'Start time' should be less than 'End Time'")
        if (prompt_end_time - prompt_start_time) > 30:
            raise gr.Error("The duration for the audio prompt should not exceed 30s")
        if vocal_track_prompt == None:
            raise gr.Error("You must provide at least a Vocal audio prompt")
    ...
    for genres_no, genres in enumerate(genres_input):
        for gen_no in range(repeat_generation):
            ...
            def save_audio(
                wav: torch.Tensor, path, sample_rate: int, rescale: bool = False
            ):
                ...
                max_val = wav.abs().max()
                wav = (
                    wav * min(limit / max_val, 1)
                    if rescale
                    else wav.clamp(-limit, limit)
                )
                torchaudio.save(
                    str(path),
                    wav,
            ...
            try:
                ...
                vocoder_mix = os.path.join(
                    vocoder_mix_dir, os.path.basename(recons_mix)
                )
...
def create_demo():
    with gr.Blocks() as demo:
        ...
        if use_icl:
            ...
            gr.Markdown(
                "<H3>- a single mixed (song/instruments) Audio track prompt</H3>"
            )
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                seed = gr.Slider(
                    0, 999999999, value=123, step=1, label="Seed (0 for random)"
                )
                with gr.Row():
                    with gr.Column():
                        ...
                        gen_status.change(
                            refresh_gallery,
                            inputs=[state],
                            outputs=[output, files_history],
                        )

        with gr.Row(visible=use_icl):  # use_icl
            with gr.Column():
                vocal_track_prompt = gr.Audio(
                    label="Audio track prompt / Vocal track prompt", type="filepath"
                )
            with gr.Column():
                instrumental_track_prompt = gr.Audio(
                    label="Intrumental track prompt (optional if Vocal track prompt set)",
                    type="filepath",
                )
        with gr.Row(visible=use_icl):
            with gr.Column():
                prompt_start_time = gr.Slider(
                    0.0, 300.0, value=0.0, step=0.5, label="Audio Prompt Start time"
                )
                prompt_end_time = gr.Slider(
                    0.0, 300.0, value=30.0, step=0.5, label="Audio Prompt End time"
                )
