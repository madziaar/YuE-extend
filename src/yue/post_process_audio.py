...
def replace_low_freq_with_energy_matched(
    ...
    # ----------------------------------------------------------
    wave_a_low = F.lowpass_biquad(wave_a, sample_rate=sr_b, cutoff_freq=cutoff_freq)
    wave_b_low = F.lowpass_biquad(wave_b, sample_rate=sr_b, cutoff_freq=cutoff_freq)
    ...
    # ----------------------------------------------------------
    wave_b_high = F.highpass_biquad(wave_b, sample_rate=sr_b, cutoff_freq=cutoff_freq)
    ...
    if wave_a_low_matched.size(1) != wave_b_high.size(1):
        print(
            f"Original lengths: a_low={wave_a_low_matched.size()}, b_high={wave_b_high.size()}"
        )
        ...
        print(
            f"After truncation: a_low={wave_a_low_matched.size()}, b_high={wave_b_high.size()}"
        )
        print(
            f"Samples truncated: {max(wave_a_low_matched.size(1), wave_b_high.size(1)) - min_length}"
        )
    ...
    print(
        f"Successfully created '{timestamped_filename}' with matched low-frequency energy."
    )
