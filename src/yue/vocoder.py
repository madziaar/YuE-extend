...
def find_matching_pairs(input_folder):
    ...
    for file in files:
        ...
        if "instrumental" in name.lower():
            ...
            if last_index != -1:
                ...
                base_name = (
                    name.lower()[:last_index]
                    + name.lower()[last_index + len("vocal") :]
                )
            else:
    ...
    for base_name in instrumental_files.keys():
        if base_name in vocal_files:
            pairs.append(
                (instrumental_files[base_name], vocal_files[base_name], base_name)
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="High fidelity neural audio codec using Vocos decoder."
    )
    parser.add_argument(
        "--input_folder",
        type=Path,
    ...
    parser.add_argument(
        "--output_base", type=Path, required=True, help="Base output folder."
    )
    parser.add_argument(
        "--resume_path",
        type=str,
        default="./final_ckpt/ckpt_00360000.pth",
    ...
    for instrumental_file, vocal_file, base_name in tqdm(pairs):
        ...
        try:
            ...
            print(e)
            print(
                f"mix {base_name} failed! inst: {instrumental_output.shape}, vocal: {vocal_output.shape}"
            )
