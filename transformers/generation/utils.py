            ...
            def get_layer_device_map(execution_device_map: Optional[dict] = None):
                ...
                if execution_device_map is None:
                    ...
                    return dict.fromkeys(
                        range(num_hidden_layers), execution_device_map[""]
                    )
                layer_device_map = {}
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        if unconditional_guidance > 0 and not plan9:
            ...
            unconditional_cache_position = (
                torch.ones_like(input_ids[0, -1:], dtype=torch.int64).cumsum(0) - 1
            )
        ...
        if plan9:
            ...
            if unconditional_guidance > 0:
                if batch_size != 1:
                    raise Exception(
                        "Conditional guidance only supported for the moment for batch size = 1"
                    )
                expanded_input_ids = torch.zeros(
                    (2, real_max_length), dtype=input_ids.dtype, device=input_ids.device
                )
                expanded_input_ids[0, :prompt_length] = input_ids
                expanded_input_ids[1, prompt_length - 1 : prompt_length] = input_ids[
                    0, -1:
                ]
                ...
                input_ids = torch.cat(
                    [
                        input_ids,
                        torch.zeros(
                            (batch_size, real_max_length - prompt_length),
            ...
            if session_cache != None and "position_ids" in session_cache:
                ...
                if unconditional_guidance > 0:
                    position_ids = torch.zeros(
                        2, max_length, dtype=torch.int32, device=input_ids.device
                    )
                    position_ids[0] = torch.arange(
                        0, max_length, dtype=torch.int32, device=input_ids.device
                    )
                    position_ids[1, prompt_length - 1 :] = position_ids[
                        0, : real_max_length - prompt_length + 1
                    ]
                    ...
                    position_ids[:] = torch.arange(
                        0, real_max_length, dtype=torch.int32, device=input_ids.device
                    )
                    start_positions = torch.zeros(
                        batch_size, dtype=position_ids.dtype, device=position_ids.device
                    )
        ...
        while self._has_unfinished_sequences(
            ...
            if plan9:
                ...
                if unconditional_guidance > 0:
                    model_inputs["unconditional_guidance"] = unconditional_guidance
                    model_inputs["unconditional_past_key_values"] = (
                        unconditional_past_key_values
                    )
                    model_inputs["unconditional_cache_position"] = (
                        unconditional_cache_position
                    )
            ...
            if plan9:
                ...
                if unconditional_guidance > 0:
                    ...
                    next_token_scores = (
                        unconditional_guidance * (scores - unconditional_scores)
                ...
                if unconditional_guidance > 0:
                    ...
                    unconditional_scores = torch.nn.functional.log_softmax(
                        unconditional_logits[:, -1], dim=-1
                    )
                    next_token_scores = (
                        unconditional_guidance * (scores - unconditional_scores)
                        + unconditional_scores
