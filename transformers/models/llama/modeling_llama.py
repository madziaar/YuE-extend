        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if not plan9:
            ...
            if unconditional_guidance > 0:
                ...
                past_seen_tokens = (
                    unconditional_past_key_values.get_seq_length()
                    if unconditional_past_key_values is not None
                    else 0
                )
                unconditional_position_ids = torch.full(
                    (inputs_embeds.shape[0], 1),
                    past_seen_tokens,
                    dtype=torch.int64,
                    device=inputs_embeds.device,
                )
                unconditional_position_embeddings = self.rotary_emb(
                    unconditional_hidden_states, unconditional_position_ids
                )
        ...
        if unconditional_guidance > 0:
            ...
            output["unconditional_past_key_values"] = (
                unconditional_past_key_values if use_cache else None,
            )
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if unconditional_guidance > 0:
            ...
            unconditional_logits = self.lm_head(
                unconditional_hidden_states[:, -num_logits_to_keep:, :]
            )
